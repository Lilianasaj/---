...
// Добавьте эту функцию для обновления потраченных сумм в бюджетах
...
// Вызывайте эту функцию при изменении транзакций
transactionModel.subscribe(() => {
  const all = transactionModel.getAll();
  chartPresenter.update(all);
  updateBudgetsSpent(); // Обновляем потраченные суммы в бюджетах
});